...
describe('schema + seeding', () => {
  ...
  it('declares the users cascade and the per-user index on pregenerated_tests', () => {
    const fks = db.prepare('PRAGMA foreign_key_list(pregenerated_tests)').all() as any[];
    const userFk = fks.find((f) => f.from === 'user_id');
    expect(userFk).toBeDefined();
    expect(userFk.table).toBe('users');
    expect(userFk.on_delete).toBe('CASCADE');

    const index = db
      .prepare(`SELECT sql FROM sqlite_master WHERE type='index' AND name='idx_pregenerated_avail'`)
      .get() as { sql: string };
    expect(index.sql).toContain('user_id');
  });
});

// Reproduces a database written before the pool became per-user: user_id bolted on with
// ALTER TABLE (so no FK, nullable) and the availability index still on the old columns.
describe('legacy pregenerated_tests migration', () => {
  ...
  // Writes a legacy-shaped database to a temp file and returns its path, so the test can
  // run the real createDb() migration path against it.
  function makeLegacyDb(): string {
    const file = path.join(os.tmpdir(), `preact-legacy-${randomUUID()}.sqlite3`);
    tempDbs.push(file);
    ...
    legacy.exec(`
      ...
      CREATE TABLE pregenerated_tests (
        id          INTEGER PRIMARY KEY AUTOINCREMENT,
      ...
      CREATE INDEX idx_pregenerated_avail ON pregenerated_tests(section, consumed);
      ALTER TABLE pregenerated_tests ADD COLUMN user_id INTEGER;
    `);
    legacy.close();
    return file;
  }

  it('rebuilds the table, preserving owned rows and their ids', () => {
    const file = makeLegacyDb();
    const legacy = new Database(file);
    legacy.prepare("INSERT INTO users (id, name) VALUES (7, 'Alice')").run();
    legacy
      .prepare("INSERT INTO pregenerated_tests (id, user_id, section, content, consumed) VALUES (?, ?, ?, ?, ?)")
      .run(41, 7, 'math', '{"a":1}', 0);
    legacy
      .prepare("INSERT INTO pregenerated_tests (id, user_id, section, content, consumed) VALUES (?, ?, ?, ?, ?)")
      .run(42, 7, 'reading', '{"b":2}', 1);
    // Legacy ownerless row, plus one pointing at a user that no longer exists.
    legacy.prepare("INSERT INTO pregenerated_tests (section, content) VALUES ('math', '{}')").run();
    legacy.prepare("INSERT INTO pregenerated_tests (user_id, section, content) VALUES (999, 'math', '{}')").run();
    expect(legacy.prepare('PRAGMA foreign_key_list(pregenerated_tests)').all()).toHaveLength(0);
    legacy.close();

    const migrated = createDb(file);

    const fks = migrated.prepare('PRAGMA foreign_key_list(pregenerated_tests)').all() as any[];
    expect(fks.find((f) => f.from === 'user_id')?.on_delete).toBe('CASCADE');

    const rows = migrated
      .prepare('SELECT id, user_id, section, content, consumed FROM pregenerated_tests ORDER BY id')
      .all();
    expect(rows).toEqual([
      { id: 41, user_id: 7, section: 'math', content: '{"a":1}', consumed: 0 },
      { id: 42, user_id: 7, section: 'reading', content: '{"b":2}', consumed: 1 },
    ]);
  ...
  it('cascades pool rows when their owner is deleted, which is what was broken', () => {
    const file = makeLegacyDb();
    const legacy = new Database(file);
    legacy.prepare("INSERT INTO users (id, name) VALUES (7, 'Alice')").run();
    legacy.prepare("INSERT INTO pregenerated_tests (user_id, section, content) VALUES (7, 'math', '{}')").run();
    legacy.close();

    const migrated = createDb(file);
    migrated.prepare('DELETE FROM users WHERE id = 7').run();
    expect(migrated.prepare('SELECT COUNT(*) AS n FROM pregenerated_tests').get()).toEqual({ n: 0 });
    migrated.close();
  });

  it('is idempotent — a second run leaves the rebuilt table alone', () => {
    const file = makeLegacyDb();
    const legacy = new Database(file);
    legacy.prepare("INSERT INTO users (id, name) VALUES (7, 'Alice')").run();
    legacy.prepare("INSERT INTO pregenerated_tests (id, user_id, section, content) VALUES (41, 7, 'math', '{}')").run();
    legacy.close();

    createDb(file).close();
    const twice = createDb(file);
    expect(twice.prepare('SELECT id, user_id FROM pregenerated_tests').all()).toEqual([{ id: 41, user_id: 7 }]);
    expect(twice.prepare('PRAGMA foreign_key_check').all()).toEqual([]);
    twice.close();
  });
});
